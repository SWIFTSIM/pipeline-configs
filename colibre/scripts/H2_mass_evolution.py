...
S17_Omega_H2 = pow(10.,S17_data[:,1])
simulation_lines.append(ax.errorbar(S17_expansion_factor, S17_Omega_H2*S17_expansion_factor**0,
...
simulation_lines.append(ax.errorbar(D20_expansion_factor, D20_Omega_H2*D20_expansion_factor**0,
                                    [D20_Omega_H2_lo, D20_Omega_H2_hi]*D20_expansion_factor**0, ls='none',
...
simulation_lines.append(ax.errorbar(1, F21_data[0]*rho_crit0,
                                    np.row_stack([F21_data[0]-F21_data[1], F21_data[2]-F21_data[0]])*rho_crit0,
                                    marker='o', label="Fletcher et. al (2021)"))

P20_data = np.genfromtxt(
    f"{arguments.config.config_directory}/{arguments.config.observational_data_directory}"
    "/data/CosmicH2Abundance/raw/Peroux2020_OmegaH2.txt")
simulation_lines.append(ax.errorbar(1./(P20_data[:,0]+1.), rho_crit0*10**P20_data[:,1],
                                    np.row_stack([10**P20_data[:,1] - 10**P20_data[:,2],
                                                  10**P20_data[:,3] - 10**P20_data[:,1]])*rho_crit0,
                                    zorder=0, fmt='o', 
...
simulation_lines.append(ax.errorbar(pow(G21_data[:,0]+1,-1), G21_data[:,3]*1e6,
