...
for color, (snapshot, name) in enumerate(zip(data, names)):
    ...
    if z < 5:
        stars_SNII_densities_by_redshift = {
            "$z < 1$": stars_SNII_densities[birth_redshifts < 1],
            "$1 < z < 3$": stars_SNII_densities[
                np.logical_and(birth_redshifts > 1, birth_redshifts < 3)
            ],
            "$z > 3$": stars_SNII_densities[birth_redshifts > 3],
        }

        gas_SNII_densities_by_redshift = {
            "$z < 1$": gas_SNII_densities[birth_redshifts < 1],
            "$1 < z < 3$": gas_SNII_densities[
                np.logical_and(birth_redshifts > 1, birth_redshifts < 3)
            ],
            "$z > 3$": gas_SNII_densities[birth_redshifts > 3],
        }
    else:
        stars_SNII_densities_by_redshift = {
            "$z < 7$": stars_SNII_densities[birth_redshifts < 7],
            "$7 < z < 10$": stars_SNII_densities[
                np.logical_and(birth_redshifts > 7, birth_redshifts < 10)
            ],
            "$z > 10$": stars_SNII_densities[birth_redshifts > 10],
        }

        gas_SNII_densities_by_redshift = {
            "$z < 7$": gas_SNII_densities[birth_redshifts < 7],
            "$7 < z < 10$": gas_SNII_densities[
                np.logical_and(birth_redshifts > 7, birth_redshifts < 10)
            ],
            "$z > 10$": gas_SNII_densities[birth_redshifts > 10],
        }
