...
catalogue_filenames = [
    ...
    for directory, catalogue in zip(
...
ax.set_ylabel('Stellar [Fe/H]$_{\mathrm{passive}}$ - Stellar [Fe/H]$_{\mathrm{active}}$')
ax.set_xscale('log')
...
for filename, name in zip(catalogue_filenames, arguments.name_list):
    ...
    sfr = catalogue.get_quantity(f"apertures.sfr_gas_{aperture_size}_kpc").to('Msun/yr')[mask]
    ...
    for i_bin in range(n_mass_bins):
        ...
        if (passive_abundance.shape[0] >= min_points_bin) and (star_forming_abundance.shape[0] >= min_points_bin):
            values[i_bin] = np.median(passive_abundance) - np.median(star_forming_abundance)
    ax.plot(centers[values!=0], values[values!=0], '-', label=f'{name} ($z={z:.1f})$')
...
for z in sorted(z_to_plot):
    for filename, label in [
        ...
        if np.sum(mask) == 0:
            continue
        ax.errorbar(10**obs_data[:, 1][mask], obs_data[:, 2][mask], 
                yerr=obs_data[:, 3][mask], label=f'{label} ($z={z:.1f})$')
