...
for snapshot_filename, name in zip(snapshot_filenames, names):
    ...
    ym1, _, _ = stats.binned_statistic(Fe_H, Mg_Fe, statistic=lambda x: np.percentile(x, 16.), bins=bins)
    ym2, _, _ = stats.binned_statistic(Fe_H, Mg_Fe, statistic=lambda x: np.percentile(x, 84.), bins=bins)
...
# We select APOGEE data file containing FeH-MgFe.
path_to_obs_data = f"{arguments.config.config_directory}/{arguments.config.observational_data_directory}"
observational_data = f"{path_to_obs_data}/data/StellarAbundances/APOGEE_data_MG.hdf5"
...
ax.annotate("APOGEE data", (-3.8, 1.7))
...
plt.savefig(f"{output_path}/stellar_abundances_FeH_MgFe_APOGEE.png")
