"""
Plots the stellar abundances ([Fe/H] vs [N/Fe]) for a given snapshot
"""
...
arguments = ScriptArgumentParser(
    description="Creates an [Fe/H] - [N/Fe] plot for stars."
)
...
for snapshot_filename, name in zip(snapshot_filenames, names):
    ...
    ym, _, _ = stats.binned_statistic(Fe_H, N_Fe, statistic="median", bins=bins)
    ym1, _, _ = stats.binned_statistic(Fe_H, N_Fe, statistic=lambda x: np.percentile(x, 16.), bins=bins)
    ym2, _, _ = stats.binned_statistic(Fe_H, N_Fe, statistic=lambda x: np.percentile(x, 84.), bins=bins)
    counts, _, _ = stats.binned_statistic(Fe_H, N_Fe, statistic="count", bins=bins)
...
# We select all Tolstoy* files containing FeH-CFe.
path_to_obs_data = f"{arguments.config.config_directory}/{arguments.config.observational_data_directory}"
observational_data = f"{path_to_obs_data}/data/StellarAbundances/APOGEE_data_N.hdf5"
...
plt.savefig(f"{output_path}/stellar_abundances_FeH_NFe_APOGEE.png")
