...
for snapshot_filename, name in zip(snapshot_filenames, names):
    ...
    ym1, _, _ = stats.binned_statistic(
        Fe_H, O_Fe, statistic=lambda x: np.percentile(x, 16.0), bins=bins
    )
    ym2, _, _ = stats.binned_statistic(
        Fe_H, O_Fe, statistic=lambda x: np.percentile(x, 84.0), bins=bins
    )
...
observational_data = f"{path_to_obs_data}/data/StellarAbundances/APOGEE_data_O.hdf5"
...
plt.savefig(f"{output_path}/stellar_abundances_FeH_OFe_APOGEE.png")
