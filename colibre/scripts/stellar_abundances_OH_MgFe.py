...
for snapshot_filename, name in zip(snapshot_filenames, names):
    ...
    ym1, _, _ = stats.binned_statistic(O_H, Mg_Fe, statistic=lambda x: np.percentile(x, 16.), bins=bins)
    ym2, _, _ = stats.binned_statistic(O_H, Mg_Fe, statistic=lambda x: np.percentile(x, 84.), bins=bins)
