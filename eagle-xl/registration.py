...
def register_star_forming_gas_oxygen_abundance(
    aperture_sizes, Z_sun, twelve_plus_log_OH_solar, minimal_twelve_plus_log_OH
):
    for aperture_size in aperture_sizes:
        sphere = getattr(soap, f"exclusive_sphere_{aperture_size}kpc")
        try:
            metal_frac_sf = sphere.star_forming_gas_mass_fraction_in_metals / Z_sun
            ...
            twelve_plus_log_OH.name = (
                f"Gas (SF) $12+\\log_{{10}}$O/H from $Z$ "
                f"(Solar={twelve_plus_log_OH_solar}) ({aperture_size} kpc)"
            )
            twelve_plus_log_OH[
...
def register_species_fractions(aperture_sizes):
    M_star_100 = soap.exclusive_sphere_100kpc.stellar_mass
    xgass_select = (M_star_100 > unyt.unyt_quantity(10**9, "Solar_Mass")) & (
        M_star_100 < unyt.unyt_quantity(10**11.5, "Solar_Mass")
    )
...
def register_stellar_velocity_dispersion():
...
register_star_forming_gas_oxygen_abundance(
    aperture_sizes,
...
register_baryon_fractions()
register_stellar_magnitudes(aperture_sizes, BAND_COLUMNS)
register_stellar_velocity_dispersion()
