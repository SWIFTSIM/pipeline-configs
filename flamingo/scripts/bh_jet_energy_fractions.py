...
def get_data(filename):
    ...
    jet_energies = np.array(data.black_holes.injected_jet_energies.value)
    lum_energies = np.array(data.black_holes.agntotal_injected_energies.value)
...
def make_single_image(filenames, names, mass_bounds, value_bounds, number_of_simulations, output_path):
    ...
    ax.set_ylabel("Black Hole Injected Jet Energy Fractions $E_{\\rm jet}/(E_{\\rm jet} + E_{\\rm th})$ ")
    ax.set_xscale('log')
    #ax.set_yscale('log')
...
if __name__ == "__main__":
    ...
    arguments = ScriptArgumentParser(description="AGN injected jet energy fractions - BH mass relation")
