...
def get_data(filename):
    ...
    values = np.array(data.black_holes.number_of_jet_particles_launched.value)
    heats = np.array(data.black_holes.number_of_heating_events.value)
    values = values / (values + heats)
...
def make_single_image(filenames, names, mass_bounds, value_bounds, number_of_simulations, output_path):
    ...
    ax.set_ylabel("Fraction Of Particles Kicked Into BH Jets  $N_{\\rm jet} / (N_{\\rm jet} + N_{\\rm th}) $")
    ax.set_xscale('log')
    #ax.set_yscale('log')
    ...
    fig.savefig(f"{output_path}/black_hole_jet_kick_fractions.png")
...
if __name__ == "__main__":
    ...
    arguments = ScriptArgumentParser(description="Fraction of AGN particles kicked into jets by AGN jet feedback - BH mass relation")
