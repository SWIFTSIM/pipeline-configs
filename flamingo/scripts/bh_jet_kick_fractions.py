...
def get_data(filename):
    ...
    try:
        ...
        masses = masses[(kicks + heatings) > 0]
        values = values[(kicks + heatings) > 0]
    except:
...
def make_single_image(
    ...
    ax.set_ylabel(
        "Fraction Of Particles Kicked Into BH Jets  $N_{\\rm jet} / (N_{\\rm jet} + N_{\\rm th}) $"
    )
    ax.set_xscale("log")
...
if __name__ == "__main__":
    ...
    arguments = ScriptArgumentParser(
        description="Fraction of AGN particles kicked into jets by AGN jet feedback - BH mass relation"
    )
