...
def get_data(filename):
    ...
    rad_effs = np.array(data.black_holes.radiative_efficiencies.value)
    values = 6.44*10**23*9*10**16*10**4*np.array(data.black_holes.accretion_rates.value)*rad_effs

    return np.array(masses.value)[rad_effs>0.0001], values[rad_effs>0.0001]
