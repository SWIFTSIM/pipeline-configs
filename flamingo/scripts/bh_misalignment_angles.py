...
def get_data(filename):
    ...
    values = np.arccos(data.black_holes.cos_accretion_disk_angle.value)/np.pi*180
...
def make_single_image(filenames, names, mass_bounds, value_bounds, number_of_simulations, output_path):
    ...
    ax.set_ylabel("Misalignment angle " + r"$\theta_\mathrm{gas,BH}$ $[\degree]$")
    ax.set_xscale('log')
    ...
    fig.savefig(f"{output_path}/black_hole_misalignment_angles.png")
...
if __name__ == "__main__":
    ...
    arguments = ScriptArgumentParser(description="Misalignment Angle - BH mass relation")
