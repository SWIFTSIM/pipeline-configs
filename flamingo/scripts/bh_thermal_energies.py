...
def get_data(filename):
    ...
    values = 2*1e53*lum_energies

    return np.array(masses.value)[values>0], values[values>0]
...
def make_single_image(filenames, names, mass_bounds, value_bounds, number_of_simulations, output_path):
    ...
    ax.set_ylabel("Black Hole Injected Thermal AGN Energies $E_{\\rm th}$ $[{\\rm erg}]$")
    ax.set_xscale('log')
    ax.set_yscale('log')
...
if __name__ == "__main__":
    ...
    arguments = ScriptArgumentParser(description="AGN injected thermal energies - BH mass relation")
