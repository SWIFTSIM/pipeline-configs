"""
Plots the star metallicity density evolution.
"""
import matplotlib
...
for snapshot_filename, stats_filename, name in zip(
    ...
    simulation_lines.append(ax.plot(scale_factor, star_Z_mass_density, zorder=10000)[0])
    simulation_labels.append(name)

# Observational data plotting

observational_data = glob.glob(
    f"{arguments.config.config_directory}/{arguments.config.observational_data_directory}/data/MetalMassDensity/*_stars.hdf5"
)
...
fig.savefig(f"{output_path}/star_metallicity_evolution.png")
